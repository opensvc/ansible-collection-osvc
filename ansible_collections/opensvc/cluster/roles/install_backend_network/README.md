...
`install_backend_network_osvc_backend_subnet_name`

The cluster backend network name.

Default value is `backend`

`install_backend_network_osvc_backend_subnet_range`

The cluster backend network range. The routed_bridge driver fragments this into `install_backend_network_osvc_backend_subnet_ips_per_node` blocks subnets.

Default value is `10.100.0.0/16`

`install_backend_network_osvc_backend_subnet_ips_per_node`

The number of allocatable ips per node on the network.

Default value is `1024`

`install_backend_network_osvc_backend_tunnel`
    ...
    - hosts: all
      tasks:
        - import_role:
          ...
          vars:
            install_backend_network_osvc_backend_subnet_name: backendnet
            install_backend_network_osvc_backend_subnet_range: 10.123.0.0/16
            install_backend_network_osvc_backend_subnet_ips_per_node: 4096
            install_backend_network_osvc_backend_tunnel: auto
